...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
    ...
    */
    ...
    ptep = get_pte(mm->pgdir, addr, 1);//(1) try to find a pte, if pte's PT(Page Table) isn't existed, then create a PT.
     if (ptep == NULL) {
             cprintf("get_pte is failed in do_pgfault\n");
             goto failed;
         }
     if (*ptep == 0) {
         if (pgdir_alloc_page(mm->pgdir, addr, perm) == NULL) { //(2) if the phy addr isn't exist, then alloc a page & map the phy addr with logical addr
                 cprintf("pgdir_alloc_page is failed in do_pgfault\n");
                 goto failed;
             }
     ...
     */
         if(swap_init_ok) {
             ...
             if (ret!=0){                        //    into the memory which page managed.
             	cprintf("swap_in is failed in do_pgfault\n");
             	goto failed;
             }
